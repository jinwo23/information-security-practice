...
if command -v pip-audit &> /dev/null; then
    ...
    echo "pip-audit не встановлений, пропускаємо"
fi

echo "5. Статичний аналіз Bandit..."
if command -v bandit &> /dev/null; then
    BANDIT_ISSUES=$(bandit -r app/ -q --severity-level high 2>/dev/null | grep -c "Issue")
    ...
    echo "bandit не встановлений, пропускаємо"
fi
